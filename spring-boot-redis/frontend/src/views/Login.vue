...
import type { LoginRequest, ApiResponse } from "../types/api"; // 引入ApiResponse类型
...
const sendSms = async () => {
  ...
  try {
  const axiosResponse = await authApi.sendSms(formData.phone); // 无需提前断言类型
  const response = axiosResponse.data as ApiResponse; // 从 data 中取实际响应
...
const login = async () => {
  ...
  try {
    const axiosResponse = await authApi.login(formData);
    const response = axiosResponse.data as ApiResponse<LoginResponseData>;
    const loginResult = response.data; // 声明并赋值 loginResult
...
.title {
  background: linear-gradient(45deg, #409eff, #57c23a);
